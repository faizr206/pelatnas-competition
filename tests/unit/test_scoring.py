...
def test_compute_submission_score_for_csv_row_count(tmp_path: Path) -> None:
    ...
    result = compute_submission_score(
        submission_type="csv",
    ...
    assert result.public_score_value == 3.0
    assert result.private_score_value == 3.0
...
""".strip(),
    ...
    assert result.metric_value == 18.0
    assert result.score_value == 18.0
    assert result.public_score_value == 18.0
    assert result.private_score_value == 18.0
